...
void esperarCuchara() { // APARTADO 1
	check_error_val(pthread_mutex_lock(&mestado), "pthread_mutex_lock (esperarCuchara)", 15);
	while (estado != condimentando)
		check_error_val(pthread_cond_wait(&condestado, &mestado), "pthread_cond_wait (esperarCuchara)", 15);
	check_error_val(pthread_mutex_unlock(&mestado), "pthread_mutex_unlock (esperarCuchara)", 15);
}


void soltarCuchara() { // APARTADO 1
	check_error_val(pthread_mutex_lock(&mestado), "pthread_mutex_lock (soltarCuchara)", 16);
	while (estado != hablando)
		check_error_val(pthread_cond_wait(&condestado, &mestado), "pthread_cond_wait (soltarCuchara)", 16);
	check_error_val(pthread_mutex_unlock(&mestado), "pthread_mutex_unlock (soltarCuchara)", 16);
}
...
void *comunicaciones(void) {
	...
	check_error_val(sockserver, "No se pudo crear el socket de comunicación con el anterior en el anilo", 3);

	servidor.sin_family = AF_INET;
	servidor.sin_addr.s_addr = htonl(INADDR_ANY);
	servidor.sin_port = htons(puerto_local);
	check_error_val(bind(sockserver, (struct sockaddr *)&servidor, sizeof(servidor)),
	...
	check_error_val(socknext, "Error creando el socket de conexion con el siguiente", 5);
	...
	check_error_val(connect(socknext, (struct sockaddr *)&next, sizeof(next)),
