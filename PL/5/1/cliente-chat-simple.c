...
void leer_y_procesar_teclado(int socketUDP)
{
    ...
    if (linea[0] == '/')
    {
        ...
        {
            // Es un comando /CHAT, hay que leer la IP y el puerto
            // y guardarlos para usarlos en el envío de mensajes
        ...
        {
            printf("Comando desconocido: %s", linea);
        }
    }
    else      // Si la linea no comienza por /, es un mensaje a enviar al peer
    {
        ...
        mensaje_a_enviar = (char *) malloc(strlen(nick) + strlen(linea) + 2);
        sprintf(mensaje_a_enviar, "%s> %s", nick, linea);
        sendto(socketUDP, mensaje_a_enviar, strlen(mensaje_a_enviar), 0,
               (struct sockaddr *) &dir_destino, sizeof(struct sockaddr_in));
